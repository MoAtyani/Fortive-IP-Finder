...
import (
	...
	"github.com/musana/fortive-ip-finder/internal/dns"
	httpClient "github.com/musana/fortive-ip-finder/internal/http"
...
type Scanner struct {
	...
	OnResult func(url string, realIP string, source string, htmlTitle string)
}
...
func (s *Scanner) Start(url string) {
	...
	if len(cfIPs) > 0 {
		primaryIP = cfIPs[0]
		isCloudflare = true
	} else if len(nonCFIPs) > 0 {
		primaryIP = nonCFIPs[0]
	} else {
		color.Yellow("[!] Could not resolve A records for %s. Continuing scan...", domain)
		if primaryIP == nil {
			color.Cyan("[+] No A records found for %s. Continuing with other techniques...", domain)
			// Emit result indicating lack of DNS info
			if s.OnResult != nil {
				s.OnResult(url, "N/A", "Info", "No A records")
			}
		}
	}
	...
	s.getTXTRecords(domain, url, primaryIP, actualHTMLTitle)

	if s.Options.Censys {
		s.censysSearch(domain, url, primaryIP, actualHTMLTitle)
	}

	if s.Options.SecurityTrails {
		s.securityTrailsSearch(domain, url, primaryIP, actualHTMLTitle)
	}

	if s.Options.Shodan {
		s.shodanSearch(domain, url, primaryIP, actualHTMLTitle)
	}

	if s.Options.Zoomeye {
		s.zoomeyeSearch(domain, url, primaryIP, actualHTMLTitle)
	}

	if s.Options.DomainList != "" && s.Options.TargetDomain != "" {
		s.checkDomainList(url, primaryIP, actualHTMLTitle)
	}
...
func (s *Scanner) securityTrailsSearch(domain, url string, cfIP net.IP, actualHTMLTitle string) {
	...
	if len(keys) == 0 {
		color.Yellow("[!] SecurityTrails API key not found in ~/.config/fortive-ip-finder.yaml")
		return
	}
	key := keys[0]
	if key == "" {
		color.Yellow("[!] SecurityTrails API key is empty in ~/.config/fortive-ip-finder.yaml")
		return
	}
...
func (s *Scanner) shodanSearch(domain, url string, cfIP net.IP, actualHTMLTitle string) {
	...
	if len(keys) == 0 {
		color.Yellow("[!] Shodan API key not found in ~/.config/fortive-ip-finder.yaml")
		return
	}
	key := keys[0]
	if key == "" {
		color.Yellow("[!] Shodan API key is empty in ~/.config/fortive-ip-finder.yaml")
		return
	}
...
func (s *Scanner) zoomeyeSearch(domain, url string, cfIP net.IP, actualHTMLTitle string) {
	...
	if len(keys) == 0 {
		color.Yellow("[!] ZoomEye API key not found in ~/.config/fortive-ip-finder.yaml")
		return
	}
	key := keys[0]
	if key == "" {
		color.Yellow("[!] ZoomEye API key is empty in ~/.config/fortive-ip-finder.yaml")
		return
	}
...
func (s *Scanner) compareTitle(url string, ip net.IP, cfIP net.IP, source string, actualHTMLTitle string) {
	...
	// Increment counter for each processed IP (real IP candidate)
	s.mu.Lock()
	s.Stats.RealIPsFound++
	s.mu.Unlock()

	if actualHTMLTitle == foundIPTitle {
		// Title matched – already counted above, could add extra handling if needed
	}

	// Always emit a result, include both titles for debugging
	displayTitle := actualHTMLTitle
	if actualHTMLTitle != foundIPTitle {
		displayTitle = actualHTMLTitle + " | " + foundIPTitle
	}
	s.printResult(url, cfIP, ip, source, displayTitle)
}
...
func (s *Scanner) printResult(url string, cfIP net.IP, realIP interface{}, source, htmlTitle string) {
	...
	if s.OnResult != nil {
		s.OnResult(url, fmt.Sprintf("%v", realIP), source, htmlTitle)
	}
}
