...
func ParseOptions() *models.Options {
	...
	createGroup(flagSet, "server", "SERVER",
		flagSet.BoolVar(&options.Serve, "serve", false, "Start the web dashboard server"),
		flagSet.IntVar(&options.Port, "port", 8080, "Port for the web dashboard server"),
	)
